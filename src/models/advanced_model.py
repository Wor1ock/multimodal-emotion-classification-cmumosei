...
class FusionPlaceholder(nn.Module):
    def forward(self, modality_vectors: list[Tensor]) -> Tensor:
        return torch.stack(modality_vectors, dim=0).mean(dim=0)


class AdvancedMultimodalModel(L.LightningModule):
    def __init__(
        self,
        num_classes: int,
        learning_rate: float,
        ...
        self.text_bow_encoder = ModalityEncoder(text_bow_dim, hidden_dim, dropout)
        self.text_embed_encoder = ModalityEncoder(text_embed_dim, hidden_dim, dropout)
        self.audio_mfcc_encoder = ModalityEncoder(audio_mfcc_dim, hidden_dim, dropout)
        self.audio_logmel_encoder = ModalityEncoder(audio_logmel_dim, hidden_dim, dropout)
        self.fusion = FusionPlaceholder()
    ...
    def encode_modalities(self, batch: dict[str, Tensor]) -> list[Tensor]:
        return [
            self.text_bow_encoder(batch["text_bow"]),
            self.text_embed_encoder(batch["text_embed"]),
            self.audio_mfcc_encoder(batch["audio_mfcc"]),
            self.audio_logmel_encoder(batch["audio_logmel"]),
        ]
